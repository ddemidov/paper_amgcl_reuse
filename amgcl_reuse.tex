...
\begin{algorithm}
    \caption{AMG V-cycle.}
    \begin{algorithmic} \label{alg:vcycle}
...
parallelized with both OpenMP and CUDA technologies. All tests were conducted
on a machine with 3.40GHz 4 core Intel Core i5-3570K CPU and NVIDIA GeForce GTX
1050 Ti GPU.
...
\Cref{tab:setup} shows cost of each of the steps from \Cref{alg:setup} for the
level set and Navier-Stokes problems relative to the complete setup time. The
data shows that the construction of the transfer operators takes significant
portion of the setup. The partial reuse strategy basically removes this step
from the setup, which allows to save 20 to 60\% of the setup cost.
...
performed once, which yields the total speedup of 22\% for the OpenMP backend
and 185\% for the CUDA-accelerated backend. However, for the Navier-Stokes
problem a preconditioner constructed for one time step is completely
unapplicable on the next time step, which is obvious from the jump in the
...
grow, and the setup time is reduced by 20\% to 60\%. The total speedup in the
case of the level set problem is 13\% and 29\% for the OpenMP and CUDA backends
...
For both strategies, resusing the AMG setup makes more sense for the CUDA
backend, where the solution time is about 4 times lower than on the OpenMP
backend. This makes the amortization strategy more important, since the
relative setup cost is that much higher.
